...
if (!isDedicated) then {
	...
	call compile preprocessFileLineNumbers format ["baseJump\lang\%1.sqf",BJ_CFG_LANG];
	BIS_fnc_halo2 = compile preprocessFileLineNumbers "baseJump\action\fn_halo.sqf";
	BJ_MONITOR = {
		sleep 0.1;
		...
		while {alive player} do {	
			if !(isNull player) then {
				...
				if ( isNil "BJ_ACTION_PREPARE" && ( _vehicle == player ) && !(_vehicle isKindOf "Air") ) then {
					BJ_ACTION_PREPARE = _vehicle addAction [ ("<t color='#FE9A2E'>" + BJ_STR_PREPARE + "</t>"),"baseJump\action\prepare.sqf",[_vehicle,_onLadder],999,false,false,"",""];
				};
				...
				if ( isNil "BJ_ACTION_JUMP" && !isNil "_readyToJump" ) then {
					...
					BJ_ACTION_NOJUMP = _vehicle addAction [ ("<t color='#FE9A2E'>" + BJ_STR_NOJUMP + "</t>"),"baseJump\action\jump.sqf",[_vehicle,0],998,false,false,"",""];
					BJ_ACTION_JUMP = _vehicle addAction [ ("<t color='#FE9A2E'>" + BJ_STR_JUMP + "</t>"),"baseJump\action\jump.sqf",[_vehicle,1],997,false,false,"",""];
				};
