...
if (dayz_combat == 1) then {
	...
	sleep 2;
	[dayz_playerUID,dayz_characterID,'Functionary1_EP1_DZ'] spawn player_humanityMorph;
	sleep 2;
	showCommandingMenu '';
	
	player addWeapon 'M1014';
	player selectWeapon 'M1014';
	player addMagazine '8Rnd_B_Beneli_74Slug';
	player addMagazine '8Rnd_B_Beneli_74Slug';
	player addMagazine 'ItemBriefcaseEmpty';
	reload player;
	
	_object = 'VolhaLimo_TK_CIV_EP1' createVehicle (position player);
