...
diag_log text "[EMS]: Loading ExecVM Functions.";
...
diag_log text "[EMS]: Loading Compiled Functions.";
// compiled functions
DZMSAISpawn = compile preprocessFileLineNumbers "\z\addons\dayz_server\EMS\Scripts\DZMSAISpawn.sqf";

diag_log text "[EMS]: Loading All Other Functions.";
...
DZMSFindPos = {
    ...
    if (_mapHardCenter AND (!(DZMSStaticPlc))) then {
        ...
        while {_findRun} do
        {
			...
			//diag_log text format ["[EMS]: DEBUG: Pos:[%1,%2] / noWater?:%3 / okDistance?:%4 / TaviHeight:%5 / BlackListed?:%6", _posX, _posY, _noWater, _okDis, _tavHeight, _isBlack];
            sleep 2;
        };
...
DZMSSetupVehicle = {
	...
	/* From Defent - Removes Ammo From Mission Vehicles If Set To True*/
	if (DEFRemVehAmmo) then {
...
DZMSWaitMissionComp = {
    ...
    diag_log text format["[EMS]: (%3) Waiting for %1/%2 Units or Less to be Alive and a Player to be Near the Objective.",(_numSpawned - _numKillReq),_numSpawned,_unitArrayName];
...
//------------------------------------------------------------------//
diag_log text format ["[EMS]: Mission Functions Script Loaded!"];
