...
// Error Check
if (!isServer) exitWith { diag_log text format ["[EMS]: <ERROR> EMS is Installed Incorrectly! EMS is not Running!"]; };
if (!isnil("DZMSInstalled")) exitWith { diag_log text format ["[EMS]: <ERROR> EMS is Installed Twice or Installed Incorrectly!"]; };

// Global for other scripts to check if EMS is installed.
DZMSInstalled = true;

diag_log text format ["[EMS]: Starting DayZ Mission System."];
...
if ( (isnil("DZAI_isActive")) && (isnil("SAR_version")) && (isnil("WAIconfigloaded")) ) then
{
	...
	diag_log text format ["[EMS]: Relations not found! Using EMS Relations."];
	...
	if (!isnil("DZAI_isActive")) then {
		diag_log text format ["[EMS]: DZAI Found! Using DZAI's Relations!"];
		DZMSRelations = DZMSRelations + 1;
	};
	if (!isnil("SAR_version")) then {
		diag_log text format ["[EMS]: SargeAI Found! Using SargeAI's Relations!"];
		DZMSRelations = DZMSRelations + 1;
	};
	if (!isnil("WAIconfigloaded")) then {
		diag_log text format ["[EMS]: WickedAI Found! Using WickedAI's Relations!"];
		DZMSRelations = DZMSRelations + 1;
	};
	// If we have multiple relations running, lets warn the user
	if (DZMSRelations > 1) then {
		diag_log text format ["[EMS]: Multiple Relations Detected! Unwanted AI Behaviour May Occur!"];
		diag_log text format ["[EMS]: If Issues Arise, Decide on a Single AI System! (DayZAI, SargeAI, or WickedAI)"];
	};
...
// Report the version
diag_log text format ["[EMS]: Currently Running Version: %1", DZMSVersion];

// Lets check for a copy-pasted config file
if (DZMSVersion != "1.1FIN") then {
	diag_log text format ["[EMS]: Outdated Configuration Detected! Please Update EMS!"];
	diag_log text format ["[EMS]: Old Versions are not supported by the Mod Author!"];
};

diag_log text format ["[EMS]: Mission and Extended Configuration Loaded!"];

// Lets get the map name for mission location purposes
DZMSWorldName = toLower format ["%1", worldName];
diag_log text format["[EMS]: %1 Detected. Map Specific Settings Adjusted!", DZMSWorldName];
...
if (DZMSEpoch) then {
	diag_log text format ["[EMS]: DayZ Epoch Detected! Some Scripts Adjusted!"];
};
