...
//Humvee Patrol
[[(_position select 0) + 40, _position select 1, 0],[(_position select 0) + 40, _position select 1, 0],50,2,"HMMWV_M1151_M2_CZ_DES_EP1_DZ",1] spawn vehicle_patrol;
 
//Turrets
[[[(_position select 0) - 10, (_position select 1) + 10, 0]],"MK19_TriPod",0.8,"FR_OHara_DZ",1,2,"","Random",true] call spawn_static;
[[[(_position select 0) + 10, (_position select 1) - 10, 0]],"M2StaticMG",0.8,"FR_OHara_DZ",1,2,"","Random",true] call spawn_static;
[[[(_position select 0) + 10, (_position select 1) + 10, 0]],"MK19_TriPod",0.8,"FR_OHara_DZ",1,2,"","Random",true] call spawn_static;
[[[(_position select 0) - 10, (_position select 1) - 10, 0]],"M2StaticMG",0.8,"FR_OHara_DZ",1,2,"","Random",true] call spawn_static;

//Heli Paradrop
[[(_position select 0), (_position select 1), 0],[7743.41, 7040.93, 0],400,"CH_47F_EP1_DZ",10,1,"Random",4,"","USMC_Soldier_Pilot","Random",True] spawn heli_para;
 
[_position,"Bandit Base"] execVM "\z\addons\dayz_server\WAI\missions\compile\markers.sqf";
// Send Top Right message to users , requires Remote message script
_hint = parseText format["<t align='center' color='#52bf90' shadow='2' size='1.75'>Jungle Force</t><br/><t align='center' color='#ffffff'>A jungle task force have set up a temporary encampment! Go and ambush it to make it yours!!</t>"];
...
// Send Top Right message to users , requires Remote message script
_hint = parseText format["<t align='center' color='#52bf90' shadow='2' size='1.75'>Survivors</t><br/><t align='center' color='#ffffff'>Congratulation Survivors, captured the Base. HOORA!!</t>"];
customRemoteMessage = ['hint', _hint];
publicVariable "customRemoteMessage";
//Send center message
...
// Send Top Right message to users , requires Remote message script
_hint = parseText format["<t align='center' color='#52bf90' shadow='2' size='1.75'>Time Up Survivors</t><br/><t align='center' color='#ffffff'>The survivors were unable to capture the base time is up!</t>"];
customRemoteMessage = ['hint', _hint];
publicVariable "customRemoteMessage";
//Send center message
